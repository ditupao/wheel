    ...
    for (int i = 0; i < ioapic_count; ++i) {
        ...
        if (i == 0) {
            ...
            for (; gsi < 16; ++gsi) {
                ...
                u32 lo  = ((gsi + 0x40) & 0xff)
                        | IOAPIC_FIXED | IOAPIC_INT_MASK | IOAPIC_PHYSICAL;
                if ((flg & POLARITY_MASK) == POLARITY_LOW)   { lo |= IOAPIC_LOW;   }
                else                                         { lo |= IOAPIC_HIGH;  }
                if ((flg & TRIGMODE_MASK) == TRIGMODE_LEVEL) { lo |= IOAPIC_LEVEL; }
                else                                         { lo |= IOAPIC_EDGE;  }
        ...
        for (; gsi < ioapic_devs[i].rednum; ++gsi) {
            u32 lo = ((0x40 + gsi) & 0xff)
                    | IOAPIC_LOW | IOAPIC_LEVEL
                    | IOAPIC_FIXED | IOAPIC_INT_MASK | IOAPIC_PHYSICAL;
            ioapic_write(ioapic_devs[i].base, IOAPIC_RED_H(gsi), 0);
            ioapic_write(ioapic_devs[i].base, IOAPIC_RED_L(gsi), lo);
        }
