...
static char * fmt_string(char * buf, char * end, char * str, u32 flags, int width, int precision) {
    int slen = strlen(str);
    if (0 < precision && precision < slen) { slen = precision; }
...
int vsnprintf(char * buf, usize size, const char * fmt, va_list args) {
    char * ptr = buf;
    char * end = ptr + size;
    if (end < ptr) { end = (char *) -1; size = end - ptr; }
    ...
    while (*fmt) {
        ...
        case 'c':
            ...
            if (ptr < end) { *ptr = (unsigned char) va_arg(args, int); }
            ++ptr;
            ++fmt;
            while (--width > 0) {
                if (ptr < end) { *ptr = ' '; }
        ...
        switch (qualifier) {
        case 'H':
            if (flags & FLG_SIGN) { num =          (char) va_arg(args, int); }
            else                  { num = (unsigned char) va_arg(args, int); }
            break;
        case 'h':
            if (flags & FLG_SIGN) { num =          (short) va_arg(args, int); }
            else                  { num = (unsigned short) va_arg(args, int); }
            break;
        case 'l':
            if (flags & FLG_SIGN) { num = va_arg(args,          long); }
            else                  { num = va_arg(args, unsigned long); }
            break;
        case 'L':
            if (flags & FLG_SIGN) { num = va_arg(args, i64); }
            else                  { num = va_arg(args, u64); }
            break;
        case 't':                   num = va_arg(args, ptrdiff_t);    break;
        case 'z':                   num = va_arg(args, size_t);       break;
...
int snprintf(char * buf, usize size, const char * fmt, ...) {
    va_list args;
    va_start(args, fmt);
    int num = vsnprintf(buf, size, fmt, args);
    va_end(args);
    return num;
}
