...
static void loapic_flushmmu_proc(int vec, int_frame_t * sp __UNUSED) {
    dbg_assert(vec == VECNUM_FLUSHMMU);
    // TODO: clear all tlb entries
    loapic_send_eoi();
}

static void loapic_svr_proc(int vec, int_frame_t * sp __UNUSED) {
    dbg_assert(vec == VECNUM_SPURIOUS);
    // no need to send EOI
}
...
__INIT void loapic_dev_add(madt_loapic_t * tbl) {
    if (cpu_installed < MAX_CPU_COUNT) {
